...
if (isset($_POST['searchName'])) {
    $searchName = $_POST['searchName']; // Get the input from the form
}
...
<head>
    ...
    <title>Performance Report Search</title>
</head>
<body>

    <h2>Search for Performance Report</h2>

    <!-- Step 5: Create the search form -->
    <form method="POST" action="">
        <label for="searchName">Enter Name to Search:</label>
        <input type="text" id="searchName" name="searchName" value="<?php echo htmlspecialchars($searchName); ?>" required>
    ...
    if ($searchName) {
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<div><strong>Name:</strong> " . $row['NAMES'] . "</div>";
                echo "<div><strong>Class:</strong> " . $row['CLASS'] . "</div>";
                echo "<div><strong>Trade:</strong> " . $row['TRADE'] . "</div>";
                echo "<div><strong>Position:</strong> " . $row['POSITION'] . "</div>";

                // Display the report card image (assuming it's base64 encoded or a file path)
                $reportImage = $row['REPORT'];
                echo "<div><strong>Report Card:</strong><br>";
                echo "<img src='data:image/jpeg;base64," . base64_encode($reportImage) . "' alt='Report Card Image' />";
                echo "</div><br>";
            }
        } else {
            echo "No records found for the search name: $searchName";
        }
    }
    ?>
